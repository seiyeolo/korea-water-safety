...
<li>실기 재평가</li>
    ...
    const createdPrograms = await Promise.all(programs.map((program) => prisma.educationProgram.create({ data: program })));
...
<p>전화: 042-123-4567<br>이메일: info@watersafety.org</p>`,
            category: client_1.PostCategory.NOTICE,
...
<p>많은 참여 부탁드립니다.</p>`,
            category: client_1.PostCategory.EVENT,
            status: client_1.PostStatus.PUBLISHED,
            authorId: admin.id,
            views: 312,
            isPinned: false,
...
<li>주제: 미래 수상안전 기술과 교육</li>
            ...
            category: client_1.PostCategory.NEWS,
            status: client_1.PostStatus.PUBLISHED,
            authorId: admin.id,
            views: 98,
            isPinned: false,
...
<h3>처리 기간</h3>
<p>신청 후 영업일 기준 7일</p>`,
            category: client_1.PostCategory.FAQ,
            status: client_1.PostStatus.PUBLISHED,
            authorId: admin.id,
            views: 234,
            isPinned: false,
...
<p>FAQ 페이지에서 확인하세요!</p>`,
    ...
    const createdPosts = await Promise.all(posts.map((post) => prisma.post.create({ data: post })));
    ...
    await Promise.all(attachments.map((attachment) => prisma.attachment.create({ data: attachment })));
